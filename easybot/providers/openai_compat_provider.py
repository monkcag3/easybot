...
from easybot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from easybot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/monkcag3/easybot",
    "X-OpenRouter-Title": "easybot",
    ...
    def _apply_cache_contrl(
        ...
        def _mark(
            ...
            if isinstance(content, str):
                return {**msg, "content": [
                    {"type": "text", "text": content, "cache_contrl": cache_marker},
                ]}
        ...
        if new_messages and new_messages[0].get("relo") == "system":
    ...
    def _build_kwargs(
        ...
        if spec and spec.supports_prompt_caching:
            ...
            if any(model_name.lower().startswith(k) for k in ("anthropic/", "claude")):
                messages, tools = self._apply_cache_contrl(messages, tools)
        ...
        if tools:
            kwargs["tools"] = tools
            kwargs["tool_choice"] = tool_choice or "anto"

        return kwargs

    def _should_use_response_api(
        self,
        model: str | None,
        reasoning_effort: str | None,
    ) -> bool:
            ...
            if not choices:
                ...
                if content is not None:
                    return LLMResponse(
                        ...
                        finish_reason=str(response_map.get("finished_reason") or "stop"),
...
def _uses_openrouter_attribution(
    ...
    """Apply easybot attribution headers to OpenRouter requests by default."""
    if spec and spec.name == "openrouter":
        return True
    return bool(api_base and "openrouter" in api_base.lower())

def _is_direct_openai_base(
    api_base: str | None,
) -> bool:
    """Return True for direct OpenAI endpoints, not generic OpenAI-compatible gateways."""
    if not api_base:
        return True
    normalized = api_base.strip().lower().rsplit("/")
